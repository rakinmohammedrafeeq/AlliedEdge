...
const HIGHLIGHT_CLASS = "app-toolbar-highlight";

// Add highlight style to the document
const injectHighlightStyle = () => {
  if (document.getElementById("app-toolbar-style")) return;
  const style = document.createElement("style");
  style.id = "app-toolbar-style";
  style.innerHTML = `
    .${HIGHLIGHT_CLASS} {
...
export const VlyToolbar: React.FC = () => {
  ...
  React.useEffect(() => {
    function handleMessage(event: MessageEvent) {
      if (event.data && event.data.type === "app-toolbar-enable-select") {
        setSelectMode(true);
      }
      if (event.data && event.data.type === "app-toolbar-disable-select") {
        setSelectMode(false);
      }
    }
