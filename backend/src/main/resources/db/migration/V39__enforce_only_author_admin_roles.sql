...
UPDATE users SET role = 'ROLE_AUTHOR' WHERE role IS NULL; -- avoid NULLs violating the constraint
...
-- Add a strict CHECK constraint.
ALTER TABLE users
    ADD CONSTRAINT ck_users_role_author_admin
    CHECK (role IN ('ROLE_AUTHOR','ROLE_ADMIN'));
